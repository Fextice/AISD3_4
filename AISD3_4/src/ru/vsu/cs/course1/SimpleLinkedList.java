...
public class SimpleLinkedList<T> implements Iterable<T> {
    ...
    public void reverse(SimpleLinkedList<T>value) {
        Stack<T> stack = new Stack<>();
        SimpleLinkedListNode ptr = new SimpleLinkedListNode((T) value);
        while (ptr.next != null) {
            stack.push((T) ptr);
            ptr = ptr.next;
        }
        while (!stack.isEmpty()) { tail = head; }
        ptr.next = null;
    }

    public void recReverse() {
        head = reverseNodes(head, tail);
    }

    private SimpleLinkedListNode reverseNodes(SimpleLinkedListNode prevNode, SimpleLinkedListNode currentNode) {
        if (currentNode == null)
            return head;
        SimpleLinkedListNode nextNode = currentNode.next;
